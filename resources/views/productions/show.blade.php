...
<div class="page-breadcrumb d-none d-sm-flex align-items-center mb-3">
    ...
    <div class="ms-auto gx-3">
        <div class="btn-group">
            @if (!in_array($contrat->etape, [2, 3]))
                @can('Transmettre une souscription')
                    <form action="{{ route('prod.transmettreContrat', $contrat->id)}}" method="post" class="submitForm">
                        @csrf
                        <button type="submit" class="btn btn-primary p-1 border-1 text-center px-2"><i class="fadeIn animated bx bx-send"></i> Transmettre</button>
                    </form>
                @endcan

                <button type="button" class="btn btn-info p-1 border-1 text-center mx-2 px-2">
                    <a href="{{ route('prod.edit', $contrat->id)}}" class="">
            ...
            <button class="btn btn-primary border-1 text-center px-2" style="font-size: 12px">
                                ...
                                <dl class="mb-4">
                                    ...
                                    <dd>
                                        @switch($contrat->modepaiement)
                                            ...
                                            @case('SOLDE') Société @break
                                            @case('ADF') Banque @break
                                            @default {{ $contrat->modepaiement ?? '--' }}
                                        @endswitch
                                    </dd>
